  ...
  def updateParameters(self):
    ...
    try:
      ...
      if refreshChoices:
        ...
        hasAreasymbol = False
        env.workspace = self.params[0].value   # This represents the RTSD Feature Dataset
        fcList = arcpy.ListFeatureClasses("SAPOLYGON", "Polygon")

        valList = list()
        badList = list()

        # Why would there be more than 1 feature classes?
        if len(fcList) > 0:
          ...
          for fc in fcList:
            ...
            if len(valList) == 0 and fcName[:3] != "QA_":
              ...
              for fld in flds:
                ...
                # AREASYMBOL should be the only allowable fieldname
