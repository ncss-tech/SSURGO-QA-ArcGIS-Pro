...
import numpy as np
import Shoehorn_multi2_9_2
import importlib
importlib.reload(Shoehorn_multi2_9_2)
from Shoehorn_multi2_9_2 import *
...
def BNodes2(MU, nodes, areas, dec):
    try:
        #======= Variables  ==========
        nodeS       = set() # []
...
def tweezer(arcs, inter, v0, MUpoly_, N, cutV, weakEggs ,min_angle, dec):
    try:
...
def Reassemble(iCur, arcs, polys, shapes, weakEggs, badEggs):
    count = 0
    postV = 0
    newShape = []
...
try:
    ...
    # %%% General Setup
    arcpy.AddMessage("Shoehorn Version 2.9.2")
    ...
    try:
        ...
        try:
            for xy, Ai, Ri, Li, t in sCur:
                ...
                I = (abs(i) >= 2)*2 or abs(i)    # v3 index, constrained 0-2
                if not t:
   ...
   # %%%% Call Tweezer & Reassemble
    if insert:
        try:
            ...
            postV, count, weakEggs, badEggs = Reassemble(iCur, arcs, polys,
                                                         shapes, weakEggs, badEggs)
            del iCur
            edit.stopOperation()
        ...
        iCur = arcpy.da.InsertCursor(MUout, [areaField, muField, 'SHAPE@'])
        postV, count, weakEggs, badEggs = Reassemble(iCur, arcs, polys, shapes,
                                                     weakEggs, badEggs)
        del iCur
    # %%%% Msg
    arcpy.SetProgressor('step', msg)
...
try:
    ...
    if weakEggs['Tweezer'] or weakEggs['Reassembly'] or\
       weakEggs['Cluster Tolerance']:
        weakE_ = weakE
        if arcpy.ListFeatureClasses(weakE_):
            arcpy.Delete_management(weakE_)
        arcpy.CreateFeatureclass_management(SFDS, weakE_, "POLYGON",
                                            '', '', '', sr)
        weakE_ = os.path.join(SFDS, weakE_)
        arcpy.AddField_management(weakE_, 'AREASYMBOL', 'TEXT', field_length=10)
        arcpy.AddField_management(weakE_, 'function', 'TEXT', field_length=25)
        iCur = arcpy.da.InsertCursor(weakE_, ['SHAPE@', 'AREASYMBOL', 'function'])
        ...
        if weakEggs['Cluster Tolerance']:
            q = oid+' IN ({})'.format(','.join(weakEggs['Cluster Tolerance']))
            sCur = arcpy.da.SearchCursor(MUin, ['SHAPE@', 'AREASYMBOL'], q)
            for egg in sCur:
                iCur.insertRow(list(egg)+['Cluster Tolerance'])
        arcpy.AddWarning("Polygon(s) collapsed, see feature class: "+weakE_)
        del iCur, sCur
    else:
    ...
    if badEggs['Exception'] or badEggs['Reassembly']:
        ...
        arcpy.CreateFeatureclass_management(SFDS, badE_, "POLYGON",
                                            '', '', '', sr)
        badE_ = os.path.join(SFDS, badE_)
        arcpy.AddField_management(badE_, 'AREASYMBOL', 'TEXT', field_length=10)
        arcpy.AddField_management(badE_, 'function', 'TEXT', field_length=25)
        iCur = arcpy.da.InsertCursor(badE_, ['SHAPE@', 'AREASYMBOL', 'function'])
        oid = arcpy.Describe(MUin).OIDFieldName
        ...
        if badEggs['Reassembly']:
            ...
            for egg in sCur:
                iCur.insertRow(list(egg)+['Reassembly'])
        arcpy.AddWarning("Polygon(s) couldn't be reassembled: see feature class: "+badE_)
        del iCur, sCur
    else:
    ...
    if insert:
        ...
        q = "AREASYMBOL IN ('"+"','".join(areas)+"')"
        q += f" AND ({createTimeField} <= timestamp '{start}' OR {createTimeField} IS NULL)"
        arcpy.MakeFeatureLayer_management(MUout, MUR_L, q)
        arcpy.DeleteFeatures_management(MUR_L)
    if not retain:
    ...
    try:
        ...
        arcpy.AddWarning("Seems an excel table with that name is open. \nEither close it or use another name.")
        arcpy.AddWarning("Otherwise, the tool ran sucessfully, you do not need run it again.")
        pass
    arcpy.AddMessage(" ")
    arcpy.AddMessage("* * * * * * * * * * * * * * * * * *")
except:
